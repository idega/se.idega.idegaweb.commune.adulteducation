...
public class CoursePackageEditor extends AdultEducationBlock {
	...
	private static final String PARAMETER_LOCALIZED_KEY = "cpe_localized_key";
	...
	private void showForm() throws RemoteException {
		...
		SubmitButton save = (SubmitButton) getButton(new SubmitButton(localize("save_package", "Save package")));
		
		table.add(getSmallHeader(localize("package_name", "Package name") + ":"), 1, 1);
		table.add(name, 2, 1);
		table.add(getSmallHeader(localize("localized_key", "Localized key") + ":"), 1, 2);
		...
		packageTable.setColumns(3);
		packageTable.setRowColor(1, getHeaderColor());
		int row = 1;
		
		packageTable.add(getLocalizedSmallHeader("course_package","Course package"), 1, row);
		packageTable.add(getLocalizedSmallHeader("localized_key","Localized key"), 2, row++);
		...
		while (iter.hasNext()) {
			...
			packageTable.add(getSmallText(coursePackage.getLocalizedKey() != null ? coursePackage.getLocalizedKey() : "-"), 2, row);
			
			if (!getBusiness().hasSchoolPackages(coursePackage)) {
				Link delete = new Link(getDeleteIcon(localize("remove_course_package", "Remove course package")));
				delete.addParameter(PARAMETER_ACTION, ACTION_REMOVE);
				delete.addParameter(PARAMETER_COURSE_PACKAGE, coursePackage.getPrimaryKey().toString());
				packageTable.add(delete, 3, row);
			}
	...
	private void storePackage(IWContext iwc) throws RemoteException {
		try {
			getBusiness().storePackage(iwc, iwc.getParameter(PARAMETER_NAME), iwc.getParameter(PARAMETER_LOCALIZED_KEY));
		}
	...
	private void removePackage(IWContext iwc) throws RemoteException {
		try {
			getBusiness().removePackage(iwc.getParameter(PARAMETER_COURSE_PACKAGE));
		}
